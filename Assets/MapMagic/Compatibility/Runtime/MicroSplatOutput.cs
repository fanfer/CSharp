...
namespace MapMagic.Nodes.MatrixGenerators {
	...
	public class MicroSplatOutput200 : BaseTexturesOutput<MicroSplatOutput200.MicroSplatLayer>
	{
		...
		public static void Finalize (TileData data, StopToken stop) 
		{
			...
			{
				float[] opacities = sortedLayers.Select(l=>l!=null ? l.Opacity : 0);
				int[] channelNums = sortedLayers.Select(l=>l!=null ? l.channelNum : 0);

				if (stop!=null && stop.stop) return;
				Color[][] colors = CustomShaderOutput200.BlendMatrices(data.area.active.rect, sortedMatrices, sortedMasks, opacities, channelNums);
			...
			if (data.globals.microSplatApplyType==Core.Globals.MicroSplatApplyType.Splats  ||  
				data.globals.microSplatApplyType==Core.Globals.MicroSplatApplyType.Both)
			{
				TerrainLayer[] tlayers = sortedLayers.Select(l=>l?.prototype);

				if (stop!=null && stop.stop) return;
				float[,,] splats3D = TexturesOutput200.BlendLayers(sortedMatrices, sortedMasks, data.area, stop);
