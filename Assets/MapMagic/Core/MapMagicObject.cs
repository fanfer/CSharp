...
namespace MapMagic.Core 
{
	...
	public class MapMagicObject : MonoBehaviour, ISerializationCallbackReceiver
	{
		public static readonly SemVer version = new SemVer(2,1,4); 
		...
		public void OnEnable ()
		{
			...
			if (terrainSettings.material == null)
			{
				...
				terrainSettings.material = new Material(shader);
			}
		...
		#if UNITY_EDITOR
		private static void SetIsPlaying (UnityEditor.PlayModeStateChange m) 
		...
		#region IMapMagic
			...
			public void Clear (Generator gen)
			{
				...
				foreach (TerrainTile tile in tiles.All())
				{
					if (tile.main?.data!=null) graph.ClearGenerator(gen, tile.main.data);
					if (tile.draft?.data!=null) graph.ClearGenerator(gen, tile.draft.data);
				}
			}
			...
			public void Purge (OutputGenerator outGen, bool main=true, bool draft=true)
			{
				foreach (TerrainTile tile in tiles.Tiles())
				{
					if (tile.main?.data != null) outGen.Purge(tile.main.data, tile.main.terrain);
					if (tile.draft?.data != null) outGen.Purge(tile.draft.data, tile.draft.terrain);
				}
			}
